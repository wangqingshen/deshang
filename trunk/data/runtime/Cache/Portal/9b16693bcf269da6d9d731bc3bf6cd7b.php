...
<header class="am-topbar">
	...
	<div class="news-wrapper">
		<?php if(is_array($posts)): foreach($posts as $key=>$vo): ?><a href="<?php echo U('article/index', array('state'=>'news','id'=>$vo['id']));?>" >
		<div class="news-item" >
			<div class="news-item-contaner">
				<div class="news-img">
					<div class="news-date">
						<em style="font-size:20px;"><?php echo ($vo["md"]); ?></em>
						<em style="font-size:14px;"><?php echo ($vo["year"]); ?></em>
					</div>
					<?php $smeta=json_decode($vo['smeta'],true); ?>
					<?php if($smeta['thumb'] != '' ): ?><img class="news-icon" src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>"/>
					<?php else: ?>
					<img class="news-icon" src="/themes/simplebootx/Public/images/news-default.png"/><?php endif; ?>
				</div>
				<div class="news-detail">
					<div class="news-desc" style="width: 654px;">
						<h1 class="news-title"><?php echo ($vo["post_title"]); ?></h1>
						<span class="news-content"><?php echo ($vo["post_excerpt"]); ?></span>
					</div>
					<div class="news-view">
						<em></em>
						<span class="news-num"><?php echo ($vo["post_hits"]); ?></span>
					</div>
				</div>
			</div>
		</div>
		</a><?php endforeach; endif; ?>
		<?php if($lists['page'] != '' ): ?><div class="news-page" style="width: 100%;background: #fff;padding-top: 30px;">
		<div class="pagination"><ul><?php echo ($lists['page']); ?></ul></div>
		</div>
		<?php else: ?><div class="news-page" style="width: 100%;background: #fff;">
...
<script type="text/javascript">
$(function(){
	$('.news-wrapper>a').find('.news-item-contaner').css('border-bottom', '1px solid #ccc');
	$('.news-wrapper>a').last().find('.news-item-contaner').css('border-bottom', '0');
})
</script>
</html>